...
| 配置请求参数 | `config.go` + `option.go` | `RequestConfig` 结构体 + `WithTopK`/`WithFrequencyPenalty` 等 |
...
- **ProviderExtra 透传** — `RequestOption` 的 `WithXxx()` 函数统一写入 `RequestConfig.ProviderExtra`
