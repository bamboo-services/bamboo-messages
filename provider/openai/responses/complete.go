...
func (p *ResponsesProvider) CompleteWithSystem(ctx context.Context, systemPrompt string, messages []provider.Message, config *provider.ChatConfig) (*provider.CompletionResult, error) {
	...
	if err != nil {
		return nil, xError.NewError(ctx, nil, "OpenAI Responses 请求参数序列化失败", false, err)
	}
	...
	if err != nil {
		return nil, xError.NewError(ctx, nil, "OpenAI Responses 非流式对话请求失败", false, err)
	}
	...
	if err != nil {
		return nil, xError.NewError(ctx, nil, "OpenAI Responses 响应体读取失败", false, err)
	}
	...
	if resp.StatusCode >= 400 {
		...
		return nil, xError.NewError(ctx, nil, errMsg, false, fmt.Errorf("http %d: %s", resp.StatusCode, string(respBody)))
	}

	// 解析响应体
	var response completeResponse
	if err := json.Unmarshal(respBody, &response); err != nil {
		return nil, xError.NewError(ctx, nil, "OpenAI Responses 响应体解析失败", false, err)
	}
